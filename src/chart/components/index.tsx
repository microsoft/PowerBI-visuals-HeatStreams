...
export default class HeatStreamsChart extends React.Component<IHeatStreamsChartProps, IHeatStreamsState> {
  constructor(...args) {
    super(...args);
    this.state = {
      panPosition: 0,
  ...
  private onClick(x: number, y: number, ctrlKey: boolean) {
    if (this.timeScrub) {
      ...
      const category = this.categoriesInView[Math.floor(y / (this.rowHeight + gap))];
      if (category) {
  ...
  private onScroll(deltaX, deltaY) {
    const panPosition = this.zoomLevel === 1 ? 0 : Math.min(0, this.state.panPosition - deltaX);
